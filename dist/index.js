...
var require_ValidationError = __commonJS({
  "node_modules/.pnpm/zod-validation-error@1.3.0_zod@3.21.4/node_modules/zod-validation-error/dist/cjs/ValidationError.js"(exports) {
    ...
    var zod = __importStar(require_lib4());
...
function unixNowSeconds(override) {
  if (override) {
    return override;
  }
...
function getGithubContext(rawGithubContext, contextOverrides) {
  ...
  return {
    ...
    jobName: (contextOverrides == null ? void 0 : contextOverrides.jobName) || fullContext.job,
    ref: fullContext.ref,
...
function extractStreamFromContext(context2) {
  const { event, jobRun, workflowRun } = context2;
...
// src/testResultSummary/types.ts
var TestType = z.enum(["go"]);
var TestResultsFileMetadataSchema = z.object({
  testType: TestType,
  filePath: z.string().nonempty()
});
var handledTestStatuses = z.enum(["pass", "fail"]);
var handledTestResultsSchema = z.object({
  Test: z.string().min(1).optional(),
  Action: handledTestStatuses,
  Elapsed: z.number().nonnegative()
});
var TestResultSchema = z.discriminatedUnion("Action", [
...
async function main() {
  if (!core4.getState(isPost)) {
    core4.saveState(isPost, true);
    core4.info("This action no-ops during main execution");
    return;
  }
  try {
    core4.startGroup("Github Context Gathering");
    const dryRun = core4.getBooleanInput("dry-run", { required: true });
    if (dryRun) {
      core4.info("Dry run enabled");
    }
    ...
    core4.endGroup();
    core4.setFailed(`Got error ${err}, failing action`);
  }
}
main();
function getLokiRequestOptions() {
...
function getTypedInput(inputKey, required = true) {
  return core4.getInput(inputKey, { required, trimWhitespace: true });
}
